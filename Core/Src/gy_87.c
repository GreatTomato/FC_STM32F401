...
//const float degToPi= 0.01745329251994329576923690768489;
	...
	void MPU6050_Read_Gyro(I2C_HandleTypeDef *I2Cx, MPU6050_gyro_vectors_t *DataStruct)
		{
			...
			DataStruct->X = ((int16_t)(recive_Bytes[0] << 8 | recive_Bytes[1]))/32.8/100*0.01745329251994329576923690768489;// /100 is for speed per dt
			DataStruct->Y = ((int16_t)(recive_Bytes[2] << 8 | recive_Bytes[3]))/32.8/100*0.01745329251994329576923690768489;// 0.0174 is deg to pi
			DataStruct->Z = ((int16_t)(recive_Bytes[4] << 8 | recive_Bytes[5]))/32.8/100*0.01745329251994329576923690768489;
			//LBS sensitivity taken from datasheet on page with Registers 67 to 72 – Gyroscope Measurements for 1000 deg/s
		}//*/
